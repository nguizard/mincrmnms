...
int pre_selection4d(float *subject1, float *subject2, float *mask, char **images1, char **images2, int *sizes, int librarysize, int num_selected, int *selection, char *outfile, VIO_BOOL verbose){
  ...
  for (i=0;i<librarysize;i++){
    ...
    _meta=read_volume(images1[i], &imagedata, sizes);     

    ssd[i].index=i;
    ssd[i].ssd=get_ssd(subject1,imagedata,mask,sizes);
    free(imagedata);
    free_meta(_meta);
  
    _meta=read_volume(images2[i], &imagedata, sizes);     

    ssd[i].index=i;
    ssd[i].ssd=ssd[i].ssd+get_ssd(subject2,imagedata,mask,sizes);
    free(imagedata);
    free_meta(_meta);
  }
  ...
  for (i=0;i<num_selected;i++){   
    selection[i] = ssd[i].index;
    if ((verbose) || (outfile!=NULL))  fprintf(fd,"%s %s %f\n",images1[selection[i]],images2[selection[i]],ssd[i].ssd);
  }
